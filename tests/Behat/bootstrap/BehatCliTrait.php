...
trait BehatCliTrait {
  ...
  /**
   * Static fixture directory, relative to the repository root.
   *
   * The generated run reads the fixtures from its own working directory, so
   * the path has to name the same directory in both trees.
   */
  protected const BEHAT_CLI_FIXTURES_PATH = 'tests/fixtures/files';
...
$suite = (new Suite('default'))
  ...
  ->addContext(PhpServerContext::class, ['webroot' => '%paths.base%/{{FIXTURES_PATH}}', 'protocol' => 'http', 'host' => '0.0.0.0', 'port' => 8888, 'debug' => TRUE]);

$profile = (new Profile('default'))
  ->withSuite($suite)
  ->withExtension(new Extension(MinkExtension::class, [
    'base_url' => 'http://nginx:8080',
    'files_path' => '%paths.base%/{{FIXTURES_PATH}}',
    'browser_name' => 'chrome',
...
EOL;
    ...
    $content = strtr($content, [
      ...
      '{{FIXTURES_PATH}}' => static::BEHAT_CLI_FIXTURES_PATH,
    ]);
  ...
  protected function behatCliCopyFixtures() {
    $fixture_path = static::BEHAT_CLI_FIXTURES_PATH;
