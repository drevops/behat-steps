...
| PHP 8.5 x Drupal 12 x `normal` / `lowest` x Behat 4 | The Behat suite passes on the next core major. See [Drupal versions](#drupal-versions). |
...
Drupal 12 is pinned to `~12.0.0-alpha1`, and its 2 legs are the whole grid that major allows:

- Drupal 12 requires PHP 8.5, so it has no PHP 8.3 or PHP 8.4 leg.
