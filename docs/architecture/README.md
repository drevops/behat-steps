...
The same reflection pass yields both halves of the package. A method carrying a `Behat\Step\*` attribute is vocabulary and goes to `STEPS.md`; a method carrying no Behat attribute at all is toolbox and goes to `HELPERS.md`, unless its docblock withdraws it with `@internal`. `TOOLBOX_CLASSES` adds `RawContext` to that second half, because a project's own step definitions are written against its lifecycle methods as much as against a trait's helpers.
