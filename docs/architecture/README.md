...
`provision_drupal12` is a third job. It builds the Drupal 12 fixture on PHP 8.5 and Behat 4 and stops once the site bootstraps, because 2 of the contrib modules the fixture installs fatal on Drupal 12 and the suite cannot run there yet. Every job carries its majors in its name, as in `Test PHP 8.5, Drupal 11, Behat 4, Deps normal` and `Provision PHP 8.5, Drupal 12, Behat 4`, and the branch ruleset requires those names.
