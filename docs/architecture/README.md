...
Drupal 12 has no job. Its fixture exists and installs, but 2 of the contrib modules it needs fatal on that major, one of them while `drush cim` is creating config entities, so the site comes up without the configuration the suite asserts on. Every job names the majors it runs, as in `Test PHP 8.5, Drupal 11, Behat 4, Deps normal`, and the branch ruleset requires those names.
